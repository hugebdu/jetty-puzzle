		...
		$srcImg.one('load', function() {
			...
			var shuffle = function(rounds, speed) {
				...
				while(i < rounds) {				
					...
					// based on its length, we can select free positions
					var choices = [];
			...
			$pieces.click(function() {
                ...
                pieceMovedCallback(source, dest);
            ...
            var pieceMovedCallback = function(source, destination) {
                if($.isFunction(listeners.pieceMoved)) {
                    listeners.pieceMoved({source: source, destination: destination})
                }
            };

            var reshuffledCallback = function(pieces) {
                if($.isFunction(listeners.reshuffled)) {
                    var permutations = $.map(pieces, function(val, idx) {
                        return {index: idx, position: $.attr(val, 'current') };
                    });
                    listeners.reshuffled(permutations);
                }
            };

            if (settings.dispatcher) {
                settings.dispatcher.move = function(id) {
                    var piece = $pieces.eq(id - 1);
                    piece.trigger('click');
                };
                settings.dispatcher.permutate = function(permutations) {
                    lock = true;
                    for (var i = 0; i < permutations.length; i++) {
                        var pair = permutations[i];
                        var p1 = $pieces.eq(pair.from - 1);
                        p1.attr('current', pair.to - 1);

                        var target = getMatrixPosition(pair.to - 1);
                        var offset = getOffset(target.row, target.col);
                        p1.animate({left: offset.left, top: offset.top}, animation.slidingSpeed, null, null);
                    }
                    lock = false;
                };
            }
